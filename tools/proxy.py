...
needed — which is the whole point, because an export table has none. The loader
uses import slots filled by Windows before DLL initialization. (A `.def` forwarder would resolve
the module name through the normal search and land back on this proxy, so it is
not usable here.)
...
extern "system" {{
    #[link_name = "__imp_{name}"]
    static SLOT_{ident}: *const ();
}}
#[unsafe(naked)]
...
def emit(real, named, ordinal_only, forwarders, out):
    used = set()
    entries = []
...
def main(argv):
    ...
    written = emit(args.dll, named, ordinal_only, forwarders, args.out)
