"""Reproduce the squad-panel binding tables from every supported build's DLLs.
...
Each build is located through its base (tools/builds.py): a signature is taken
where the point is in the base, the neighbour that differs least.
"""
import hashlib
import builds
...
def choosers(source, target, drag_start):
    """The squad and vehicle panels' drop-target choosers, as the drag-start
    helper (at `drag_start` in `source`) calls them: a direct call right after
    `mov rcx, [reg + 0x2c0]` (squad) or `[reg + 0x2c8]` (vehicle). Each walks its
    five upgrade cards and picks the first visible empty one, or one bound to an
    upgrade the item conflicts with. Returns them and the helper in `target`."""
...
def anchors_of_reference(source):
    """Where each signature is taken in the reference: the function, or for a
...
def table(source, target, anchors):
    """(the target's bindings, the target's anchors), each found by a signature
    taken at `anchors` in `source`, the target's base."""
    result, found = {}, {}
...
def generate():
    ...
    located = {}  # build name -> (module, anchors)
    for build in sorted(builds.supported(), key=lambda b: len(b.lineage())):
        path = build.require().game
        target = Module(path)
        if build.base is None:
            source, anchors = target, anchors_of_reference(target)
        else:
            source, anchors = located[build.base.name]
        entries, located_here = table(source, target, anchors)
        located[build.name] = target, located_here
        out.append('    Build {')
